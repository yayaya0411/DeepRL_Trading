...
stock_sigmoid = stock_sigmoids(stock_name)
...
agent = model.Agent(state_dim=22 + 3, balance=initial_balance,model_name=model_name)
...
for e in tqdm.tqdm(range(1, num_episode + 1)):
    ...
    state = generate_combined_state(0, window_size, stock_prices, stock_margin, stock_sigmoid, agent.balance, len(agent.inventory)) 

    for t in range(1, trading_period + 1):
        if t % 1000 == 0:
            logging.info(f'\n-------------------Period: {t}/{trading_period}-------------------')
        reward = 0
        next_state = generate_combined_state(t, window_size, stock_prices, stock_margin, stock_sigmoid, agent.balance, len(agent.inventory))
        ...
        # execute position
        # logging.info(f'Step: {t}\tHold gnal: {actions[0]:.4} \tBuy signal: {actions[1]:.4} \tSell signal: {actions[2]:.4}')
        logging.info(f'Step: {t}\tBuy signal: {actions[0]:.4} \tSell signal: {actions[1]:.4}')
        ...
        if action == 2: # hold
            execution_result, reward = hold_v2(agent,stock_prices,t,actions)
